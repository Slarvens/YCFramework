...
namespace YCTable.Handlers.Header {
    public class HeaderDictHandler : HeaderHandlerBase {
        ...
        protected override void validate_node(Node node, AstProcessor ap) {
            // check that dictionary key type is not nullable
            var firstChild = node.get_real_children().FirstOrDefault();
            if (firstChild != null) {
                var keyHeader = firstChild.build_child(ap);
        ...
        public override IHeader build_node(Node node, AstProcessor ap) {
            ...
            IHeader k = parts.Count > 0 ? parts[0].build_child(ap) : new HeaderBasic("unknown", null);
            IHeader v = parts.Count > 1 ? parts[1].build_child(ap) : new HeaderBasic("unknown", null);
